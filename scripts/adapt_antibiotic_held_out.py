...
RUN_PLAN_PATH = (
    PROJECT
    / "metadata/final_transfer/nested_loso_v1/drug_held_out_few_shot_v1/"
...
zero177 = load_module(SCRIPT177_PATH, "frozen_zero_shot_177_for_drug_held_out_few_shot")
...
def load_inputs() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if len(plan) != EXPECTED_RUNS or plan["run_id"].duplicated().any():
        raise RuntimeError(f"Invalid drug-held-out few-shot run plan: {len(plan)}")
    ...
    if sorted(plan["seed"].unique()) != list(MODEL_SEEDS):
        raise RuntimeError("Unexpected drug-held-out few-shot seeds.")
    ...
    query = query.loc[query["target_protocol"].eq("drug_held_out")].copy()
    support = support.loc[support["target_protocol"].eq("drug_held_out")].copy()
...
def execute_run(
    ...
    if model_kind == "target_only_full_support":
        query_id_set = set(query_ids)
        support_ids = target_observations.loc[
            ~target_observations["observation_id"].astype(str).isin(
                query_id_set
            ),
            "observation_id",
        ].astype(str).tolist()
    ...
    if not query_ids or not support_ids:
        raise RuntimeError(
            f"Empty query/support membership: {run_id}"
        )

    if set(query_ids).intersection(support_ids):
        raise RuntimeError(
            f"Query/support observation leakage: {run_id}"
        )

    query = target_lookup.loc[
        query_ids
    ].reset_index()
    support = target_lookup.loc[
        support_ids
    ].reset_index()

    held_out_antibiotic = str(
        row["held_out_antibiotic"]
    )

    if query["normalized_antibiotic"].nunique() != 1:
        raise RuntimeError(
            f"Drug-held-out query is not single-drug: {run_id}"
        )

    observed_query_drug = str(
        query["normalized_antibiotic"].iloc[0]
    )

    if observed_query_drug != held_out_antibiotic:
        raise RuntimeError(
            "Held-out-drug metadata mismatch: "
            f"{run_id}; plan={held_out_antibiotic}; "
            f"query={observed_query_drug}"
        )

    if held_out_antibiotic in set(
        support["normalized_antibiotic"].astype(str)
    ):
        raise RuntimeError(
            f"Held-out drug appears in target support: {run_id}"
        )

    expected_other_drugs = (
        target_observations[
    ...
    if (
        ...
        raise RuntimeError(
            "Target support does not retain every non-query drug: "
            f"{run_id}"
        )
    ...
    if model_kind == "target_only_full_support":
        ...
        if len(support) != expected_full_support:
            raise RuntimeError(
                "Full all-other-drugs support size mismatch: "
                f"{run_id}"
            )
    ...
    summary: dict[str, Any] = {
        ...
        "query_id": query_id,
        "held_out_antibiotic": held_out_antibiotic,
        "source_mic_supervision_status": str(
            row["source_mic_supervision_status"]
        ),
        ...
        "query_label_use": "held-out drug evaluation only",
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument("--budgets", nargs="+", type=int, default=[1, 5, 10, 100])
...
def main() -> None:
    ...
    if plan.empty:
        raise RuntimeError("No drug-held-out few-shot runs selected.")
    ...
    zero_plan_lookup = zero_plan.set_index(
        "run_id"
    )
    if not zero_plan_lookup.index.is_unique:
        raise RuntimeError(
            "Duplicate zero-shot run IDs."
        )
    ...
    for _, row in plan.iterrows():
        ...
        if args.max_new_runs > 0 and new_runs >= args.max_new_runs:
            print("STATUS: SCRIPT 186 PARTIAL DRUG-HELD-OUT FEW-SHOT RUN COMPLETE")
            return

    print("STATUS: SCRIPT 186 SELECTED DRUG-HELD-OUT FEW-SHOT PARTITION COMPLETE")
