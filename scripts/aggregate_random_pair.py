...
AGGREGATE_ROOT = (
    PROJECT
    / "results/tables/final_transfer/nested_loso_v1/"
      "random_pair_few_shot_aggregate_v1"
)
...
SUCCESS_FREEZE = (
    PROJECT
    / "metadata/final_transfer/nested_loso_v1/"
      "script181_successful_random_pair_few_shot_aggregation_core_sha256.txt"
)
...
def seed_average_by_fold(
    ...
    if not output["seed_count"].eq(3).all():
        raise RuntimeError("Not every random-pair fold has three model seeds.")
    return output
...
def fold_mean_sd(
    ...
    if not output["fold_count"].eq(5).all():
        raise RuntimeError("Not every random-pair summary has five folds.")
    return output
...
def main() -> None:
    ...
    all_runs_path = AGGREGATE_ROOT / "all_random_pair_few_shot_run_metrics.tsv"
    all_per_drug_path = AGGREGATE_ROOT / "all_random_pair_few_shot_per_antibiotic_metrics.tsv"
    ...
    scratch_summary_path = AGGREGATE_ROOT / "target_only_scratch_random_pair_mean_sd.tsv"
    pretrained_summary_path = AGGREGATE_ROOT / "pretrained_few_shot_random_pair_mean_sd.tsv"
    ...
    per_drug_summary_path = AGGREGATE_ROOT / "per_antibiotic_random_pair_mean_sd.tsv"
    ...
    multisource_summary_path = AGGREGATE_ROOT / "multisource_gain_random_pair_mean_sd.tsv"
    ...
    protocol = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "value": "five random-pair query folds",
            },
            {
                "item": "seed_handling",
    ...
    input_manifest_path = AGGREGATE_METADATA_ROOT / "script181_input_manifest.tsv"
    ...
    print("===== RANDOM-PAIR PRETRAINED FEW-SHOT SUMMARY =====")
    print(
    ...
    print("STATUS: SCRIPT 181 RANDOM-PAIR FEW-SHOT RESULTS AGGREGATED AND FROZEN")
