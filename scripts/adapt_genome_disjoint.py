...
zero177 = load_module(SCRIPT177_PATH, "frozen_zero_shot_177_for_genome_disjoint_few_shot")
...
def load_inputs() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if len(plan) != EXPECTED_RUNS or plan["run_id"].duplicated().any():
        raise RuntimeError(f"Invalid genome-disjoint few-shot run plan: {len(plan)}")
    ...
    if sorted(plan["seed"].unique()) != list(MODEL_SEEDS):
        raise RuntimeError("Unexpected genome-disjoint few-shot seeds.")
    ...
    query = query.loc[query["target_protocol"].eq("genome_disjoint")].copy()
    support = support.loc[support["target_protocol"].eq("genome_disjoint")].copy()
...
def execute_run(
    ...
    if query["normalized_antibiotic"].nunique() != target_observations["normalized_antibiotic"].nunique():
        raise RuntimeError(f"Genome-disjoint query lacks target antibiotics: {run_id}")
...
def main() -> None:
    ...
    if plan.empty:
        raise RuntimeError("No genome-disjoint few-shot runs selected.")
    ...
    for _, row in plan.iterrows():
        ...
        if args.max_new_runs > 0 and new_runs >= args.max_new_runs:
            print("STATUS: SCRIPT 183 PARTIAL GENOME-DISJOINT FEW-SHOT RUN COMPLETE")
            return

    print("STATUS: SCRIPT 183 SELECTED GENOME-DISJOINT FEW-SHOT PARTITION COMPLETE")
