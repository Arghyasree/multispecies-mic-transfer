...
def load_backend():
    ...
    specification = importlib.util.spec_from_file_location(
        "corrective_low_rank_backend_143",
        BACKEND_PATH,
    )
...
class ControlledLowRankGenomeNetwork(nn.Module):
    """Projected genome multiview base plus low-rank interaction residual."""
    ...
    def __init__(
        ...
        if self.low_rank <= 0:
            raise RuntimeError(
                "Low-rank interaction rank must be positive."
            )
    ...
    def forward(
        ...
        if len(drug_views) != 1:
            raise RuntimeError(
                "Low-rank screen requires exactly one Morgan drug view."
            )
        ...
        if drug_view_id.lower() != "morgan":
            raise RuntimeError(
                "Low-rank screen requires Morgan; "
...
def load_configuration_registry() -> dict[str, dict[str, Any]]:
    ...
    if len(frame) != 12:
        raise RuntimeError(
            f"Expected 12 low-rank configuration rows; "
    ...
    for record in frame.to_dict(
        ...
        if representation in registry:
            raise RuntimeError(
                f"Duplicate low-rank representation: {representation}"
            )
...
def build_model(
    ...
    if CURRENT_SPEC is None:
        raise RuntimeError(
            "Current low-rank specification is unset."
        )
    ...
    if (
        ...
        raise RuntimeError(
            "Low-rank screen requires projected-concatenation "
            "genome-drug architecture."
        )

    if len(drug_matrices) != 1:
        raise RuntimeError(
            "Low-rank screen requires exactly one Morgan matrix."
        )
    ...
    model = ControlledLowRankGenomeNetwork(
        genome_dimension=(
...
def patch_run_outputs(
    ...
    additions = {
        ...
        "corrective_analysis_stage": (
            "Stage_B_low_rank_interaction_rank_screen"
        ),
    }
...
def execute_run(
    ...
    if representation not in CONFIG_BY_REPRESENTATION:
        raise RuntimeError(
            f"Unregistered low-rank representation: {representation}"
        )
...
def aggregate_completed_runs() -> None:
    ...
    metadata = (
        plan[
            ...
            if "model_parameter_count" in plan.columns
            else [
                "configuration_id",
                "outer_target_code",
                "shared_hp_id",
    ...
    # Recover parameter count from one completed run per configuration.
    ...
    if parameter_records:
        ...
        if (
            "model_parameter_count_from_run"
            in configuration.columns
        ):
            configuration[
                "model_parameter_count"
            ] = configuration[
    ...
    if len(selected_rank) != 3:
        raise RuntimeError(
            "Expected one selected low-rank configuration per outer target."
        )
        ...
        [
            ...
            },
            {
                ...
                "value": (
                    ...
                    "target; worst-direction is secondary and only an exact-"
                    "tie resolver"
                ),
            },
            {
            ...
            },
            {
                ...
                "value": (
                    "total trainable plus non-trainable tensor parameter count "
                    "reported for each rank candidate"
                ),
            },
            {
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=0,
        help=(
            "Run one rank only. Zero means all preregistered ranks."
        ),
    )
    parser.add_argument(
        "--seeds",
...
def main() -> None:
    ...
    if arguments.aggregate_only:
        aggregate_completed_runs()
        print(
            "STATUS: SCRIPT 163 AGGREGATE-ONLY COMPLETE"
        )
        return

    if arguments.dry_run:
        print(
            "STATUS: SCRIPT 163 DRY RUN COMPLETE"
        )
        return
    ...
    for row in plan.to_dict(
        ...
        if (
            ...
            print(
                "STATUS: SCRIPT 163 PARTIAL RUN COMPLETE"
            )
            return

    if arguments.worker_only:
        print(
            "STATUS: SCRIPT 163 WORKER PARTITION COMPLETE"
        )
        return
    ...
    print(
        "STATUS: SCRIPT 163 CORRECTIVE "
        "LOW-RANK INTERACTION SCREEN COMPLETE"
    )
